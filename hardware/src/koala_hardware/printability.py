...
"""Surface-only printability screen (DEC-29 corrects DEC-24's proof claim).
...
A part is judged in its declared print orientation:
  ...
  overhang_area - area of down-facing faces steeper than the self-support
                  limit, excluding the bed itself (inspect these in a slicer)

Overhang convention: `angle` is measured from straight-down, so a horizontal
ceiling = 0 deg and a vertical wall = 90 deg. The 45-degree threshold is a
heuristic; this neither detects every floating start nor establishes strength.
"""
import numpy as np
import trimesh
...
MIN_BED_AREA = 300.0    # mm^2 - screening threshold, not an adhesion model
MAX_OVERHANG_AREA = 800.0  # mm^2 - screening threshold, not support certification
...
def check(mesh: trimesh.Trimesh) -> tuple[bool, str, dict]:
    ...
    verdict = "; ".join(problems) if problems else "geometry screen passed; slice/strength unverified"
