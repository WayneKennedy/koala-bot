...
def prune(current: set[str]) -> list[str]:
    ...
    """
    keep = current | {"koala-quadruped", "koala-upright", "koala-walking"}
    removed = []
...
def write_bom(bom: list[dict]) -> bool:
    ...
    def configured(configuration):
        return [r | {'qty':r.get('configuration_qty',{}).get(configuration,r['qty'])}
                for r in all_parts
                if configuration in r.get('configurations',('walking','wheeled'))]
    bom=configured('walking')
    wheeled=configured('wheeled')
    ...
    alternatives=[r for r in wheeled if r['name'] not in {p['name'] for p in bom}]
    rows += ['', '**Wheeled alternative (exchange at both knees).** Replace the two '
             '`foot_shank` prints and their two rear TPU pads with the following prints. '
             'Keep the two front pads and the shared upper chassis. Fit the retained '
             'motor/hub/wheel pairs. Quantities below are alternatives, excluded from '
             'the walking total.', '',
             '| Alternative print | Ver | Qty | Printable | Material | Size (mm) |',
             '|---|---|---:|---|---|---|']
    rows += [f"| `{r['name']}` | v{r['version']} | {r['qty']} | {r['printable']} | {r['material']} | "
             + ' x '.join(f'{x:.0f}' for x in r['size']) + ' |' for r in alternatives]
    wheel_qty=sum(r['qty'] for r in wheeled if not r['name'].startswith('coupon'))
    rows += ['',f'Wheeled configuration: **{wheel_qty} physical prints**; walking configuration: **{tot_qty}**.']

    hardware = Counter()
    for r in bom:
        if not r['name'].startswith('coupon'):
            hardware.update({key: count*r['qty'] for key, count in r.get('fasteners', {}).items()})
    rows += ['', '**Walking chassis fastening schedule, derived from the same builders.** '
    ...
    wheel_hardware=Counter()
    for r in wheeled:
        if not r['name'].startswith('coupon'):
            wheel_hardware.update({key:count*r['qty'] for key,count in r.get('fasteners',{}).items()})
    rows += ['', '**Fastener changes for the wheeled configuration:**', '',
             '| Fastener | Walking | Wheeled |', '|---|---:|---:|']
    rows += [f'| {key} | {hardware[key]} | {wheel_hardware[key]} |'
             for key in sorted(hardware.keys() | wheel_hardware.keys()) if hardware[key]!=wheel_hardware[key]]
    ...
    if sliced:
        preamble = (f"Figures with a matching STL hash are **slicer estimates** using "
                    ...
                    f"`docs/design/front-redesign/slices.json`, plus "
                    f"`docs/design/walking/slices.json`); changed or unsliced "
    ...
    block = (f"{BOM_BEGIN}\n\n*Generated by `koala_hardware.export` - do not "
             f"hand-edit. Walking first build (DEC-62); coupons and alternative shanks are excluded from the total.*\n\n"
             f"{preamble}\n\n" + "\n".join(rows) + f"\n\n{BOM_END}")
...
def main():
    ...
    for builder in all_builders():
        ...
        for name, solid in variants:
            ...
            bom[-1].update({key:spec[key] for key in ('configurations','configuration_qty') if key in spec})
            if not ok:
