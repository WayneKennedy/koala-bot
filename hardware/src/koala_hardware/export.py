...
print orientation, fails the support-free test, or violates a critical
...
def write_bom(bom: list[dict]) -> bool:
    ...
    if all_structural_sliced:
        total_filament = f"{tot_vol * PETG_DENSITY:.0f} g"
        total_time = _hm(tot_secs)
    else:
        total_filament = f"~{tot_solid * PETG_DENSITY:.0f} g (solid max)"
        total_time = "-"
    ...
    if sliced:
        preamble = (f"Figures with a matching STL hash are **measured** with "
                    ...
                    f"(`koala_hardware.slice_remote`); changed or unsliced "
                    f"parts fall back to a labelled solid-geometry upper "
                    f"bound. All structural parts printed solid would be "
                    f"{tot_solid * PETG_DENSITY:.0f} g.")
    else:
        preamble = ("No current hash-matched slice results, so filament is the "
                    "**solid-geometry upper bound**, not a setting — real use "
                    "is roughly half. Run `koala_hardware.slice_remote` for "
                    "measured figures.")
