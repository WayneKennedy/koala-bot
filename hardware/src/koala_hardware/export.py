...
def write_bom(bom: list[dict]) -> bool:
    ...
    sliced = (json.loads(cache_file.read_text()) if cache_file.exists() else {})

    rows = ["| Part | Qty | Size (mm) | Filament | Print time | Print notes |",
            "|------|-----|-----------|----------|-----------|-------------|"]
    ...
    for r in sorted(bom, key=lambda r: (r["name"].startswith("coupon"),
        ...
        if r["bed_area"] < 1500 and r["size"][2] > 50:
            note.append("brim (tall, small footprint)")
        if r["overhang"] > 0:
            note.append(f"{r['overhang']:.0f} mm2 self-supporting overhang")

        s = sliced.get(r["name"])
        if s:
            fil = f"{s['cm3'] * PETG_DENSITY * r['qty']:.0f} g"
            tim = s["time"]
        else:
            fil = f"~{r['vol'] * PETG_DENSITY * r['qty']:.0f} g (solid max)"
            tim = "-"
    ...
    rows.append(f"| **Structural total** | **{tot_qty}** | | "
                f"**{tot_vol * PETG_DENSITY:.0f} g** | **{_hm(tot_secs)}** | |")

    if sliced:
        preamble = (f"Filament and time are **measured**, by slicing each STL "
                    f"with the standing PETG profile on the print host "
                    f"(`koala_hardware.slice_remote`). For scale, the same "
                    f"parts printed *solid* would be "
                    f"{tot_solid * PETG_DENSITY:.0f} g — infill is doing its "
                    f"job.")
    else:
        preamble = ("No slice cache present, so filament is the **solid-"
                    "geometry upper bound**, not a setting — real use is "
                    "roughly half. Run `koala_hardware.slice_remote` for "
                    "measured figures.")
