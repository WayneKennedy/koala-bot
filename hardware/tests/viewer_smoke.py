...
with sync_playwright() as p:
    ...
    assert '85 mm thigh' in page.locator('#detail').inner_text()
    thigh_status=page.evaluate('meshes.find(m=>m.visible).userData.printable')
    assert thigh_status in ('unknown','assumed','proven')
    assert 'printable: '+thigh_status in page.locator('#hud').inner_text()
