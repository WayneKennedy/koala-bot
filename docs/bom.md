...
**Status, 2026-09-21:** the generated tables describe the **walking first build**:
two footed rear shanks, four identical TPU pads and no fitted rear drive hardware.
They also list the replacement shanks and fastener changes for the wheeled
configuration (DEC-62). Bought drive parts remain allocated to the end goal.
Root socket v1 is physically proven,
...
now distinguish the wheeled configuration from the first walking build. Foot traction, head/neck, guards
...
*Generated by `koala_hardware.export` - do not hand-edit. Walking first build (DEC-62); coupons and alternative shanks are excluded from the total.*

Figures with a matching STL hash are **slicer estimates** using the recorded PETG/TPU review settings (`docs/design/manufacturing/slices.json` and `docs/design/front-redesign/slices.json`, plus `docs/design/walking/slices.json`); changed or unsliced parts fall back to a labelled solid-geometry upper bound. All structural parts printed solid would be 1013 g.
...
| `foot_shank_left` | v1 | 1 | unknown | PETG | 49 x 103 x 21 | 30 g | 2h 59m 44s | 1041 mm2 flagged overhang; inspect slice |
| `foot_shank_right` | v1 | 1 | unknown | PETG | 49 x 103 x 21 | 30 g | 2h 59m 49s | 1041 mm2 flagged overhang; inspect slice |
...
| `front_contact_pad` | v1 | 4 | assumed | TPU | 32 x 32 x 20 | 30 g | 1h 11m 17s | 4 mm2 flagged overhang; inspect slice |
...
| **Structural total** | | **28** | | | | **~1013 g (solid max)** | **-** | |

**Wheeled alternative (exchange at both knees).** Replace the two `foot_shank` prints and their two rear TPU pads with the following prints. Keep the two front pads and the shared upper chassis. Fit the retained motor/hub/wheel pairs. Quantities below are alternatives, excluded from the walking total.

| Alternative print | Ver | Qty | Printable | Material | Size (mm) |
|---|---|---:|---|---|---|
| `shank_right` | v2 | 1 | assumed | PETG | 123 x 46 x 49 |
| `shank_left` | v2 | 1 | assumed | PETG | 123 x 46 x 49 |

Wheeled configuration: **26 physical prints**; walking configuration: **28**.

**Walking chassis fastening schedule, derived from the same builders.** Candidate lengths require rig checks; excludes coupon hardware, head/neck mechanisms and supplier-specific wheel/hub fixings.
...
**Fastener changes for the wheeled configuration:**

| Fastener | Walking | Wheeled |
|---|---:|---:|
| M3 nut | 12 | 10 |
| M3 plain washer | 16 | 14 |
| M3x20 TPU-pad screw | 4 | 2 |
| M3x8 motor-face screw (depth to verify) | 0 | 12 |
