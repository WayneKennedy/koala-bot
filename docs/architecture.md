...
| Rear ankle wheels (DEC-43; fitted after walking, DEC-62) | **12V geared DC** (37D-class) + encoder | continuous rotation + torque |
| TPU feet (DEC-43/62) | Fixed rounded contacts; no ankle actuator | all four limbs in the first walking build; front pair in the wheeled build |
...
DEC-62's initial quadruped walking build omits the rear drive hardware and
uses the same twelve limb servos with interchangeable footed shanks. Motor
bring-up and balance follow walking; the two-tier architecture is retained.
